/**
 * Test script to verify Bedrock access with Amazon Nova models (correct format)
 * Run with: npx tsx apps/open-swe/src/test-bedrock-nova.ts
 */
...
// Test models - Amazon Nova models (try different formats)
const testModels = [
...
async function testBedrockModel(modelId: string) {
  ...
  try {
    ...
    console.log("📤 Sending test message with HumanMessage format...\n");

    // Use HumanMessage format for Nova models
    const testMessage = new HumanMessage("Say 'OK' if you can read this message.");
    ...
    } else if (error.message.includes("Malformed input") || error.message.includes("extraneous key")) {
      console.log("⚠️  ISSUE: Format incompatibility");
      console.log("   Model may require different API format\n");
...
async function runTests() {
  console.log("\n" + "=".repeat(60));
  console.log("🚀 BEDROCK ACCESS TEST - Amazon Nova Models (Correct Format)");
  console.log("=".repeat(60));
  console.log(`\nConfiguration:`);
  console.log(`   Region: ${region}`);
  ...
  if (successCount > 0) {
    ...
    console.log("💡 Note: Models may be accessible but require different API format.\n");
  }
}
